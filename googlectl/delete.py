...
class UserDelete(Command):
    ...
    def take_action(self, parsed_args):
        ...
        response = client.yes_no(
            "Are you sure to delete {} ?".format(parsed_args.email))
        if response:
...
class GroupDelete(Command):
    ...
    def take_action(self, parsed_args):
        client = googlectl.libgooglectl.Client()
        response = client.yes_no(
            "Are you sure to delete {} ?".format(parsed_args.groupkey))
        if response:
...
class GroupMemberDelete(Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('email', nargs=None, default=None)
        parser.add_argument('--groupkey', nargs=None,
                            required=True, default=None)
        return parser

    def take_action(self, parsed_args):
        client = googlectl.libgooglectl.Client()
        response = client.yes_no("Are you sure to delete {} from {} ?".format(
            parsed_args.email, parsed_args.groupkey))
